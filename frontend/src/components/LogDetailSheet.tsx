                                            ...
                                            <div className="space-y-6 pt-4">
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl">
                                                        ...
                                                        <p className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">{log.status}</p>
                                                    </div>
                                                    <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl">
                                                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Risk Score</p>
                                                        <p className="text-sm font-semibold text-rose-600 dark:text-rose-400">{log.risk_score.toFixed(2)}</p>
                                                    </div>
                                                </div>

                                                {/* Metadata */}
                                                <div className="space-y-3">
                                                    <h3 className="text-sm font-bold text-slate-900 dark:text-slate-50 border-b dark:border-slate-800 pb-2">Context</h3>
                                                ...
                                                {/* Redacted Data */}
                                                <div>
                                                    <h3 className="text-sm font-bold text-slate-900 dark:text-slate-50 border-b dark:border-slate-800 pb-2 mb-3">Detected Entities</h3>
                                                ...
                                                <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
                                                    <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
                                                        <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">PII Rehydration Map</h3>
                                                        <button
                                                            onClick={handleReveal}
                                                            disabled={isRevealing || revealedData !== null}
                                                            className="flex items-center gap-1.5 text-[10px] text-primary hover:text-primary/80 font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                                        >
                                                        ...
                                                        <div className="space-y-2">
                                                            {Object.entries(revealedData).map(([token, value]) => (
                                                                <div key={token} className="flex items-center justify-between group">
                                                                    ...
                                                                    <span className="text-[10px] font-mono text-white bg-slate-800 px-1.5 py-0.5 rounded border border-slate-700">{value}</span>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    ) : (
                                                        <div className="text-xs text-slate-500 leading-relaxed font-mono italic">
                                                ...
                                                <div className="pt-6">
                                                    <button className="w-full bg-slate-900 dark:bg-slate-50 text-white dark:text-slate-950 py-3 rounded-xl font-bold hover:opacity-90 transition-opacity">
                                                        Audit Trail Report
                                                    </button>
                                                </div>
                                            </div>
